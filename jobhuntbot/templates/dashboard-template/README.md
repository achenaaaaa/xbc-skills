...
- `current_stage`: free-text label for where an already-submitted application currently stands (e.g. "笔试", "二面"). Set by the dashboard's calendar "add/edit event" action, verbatim as typed.
- `job_description`: full job description text from the official posting, copied verbatim.
...
- `job_description`: full JD text from official posting.
...
- Use `Needs user` when a missing high-impact fact blocks the decision.
